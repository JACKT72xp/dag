...
default_args = {
    ...
    "start_date": datetime(2021, 12, 13, 20, 0),
...
#         json_data =json.loads(dumps(list_cur, indent = 2))
    ...
    def comparate_secondary_mysql_equals(df_mysql,comparate,old):
        ...
        try:
            print(both[both['platform_deviceID']=='ORBITHB600301']['concat_key_generate_secondary'],'platform_deviceIDplatform_deviceIDplatform_deviceIDplatform_deviceIDconcat_key_generate_secondarytry')
            print(df_mysql[df_mysql['mysql_siteId']=='ORBITHB600301']['concat_key_generate_secondary'],'mysql_siteIdconcat_key_generate_secondary')
            both['exist_mysql_secondary'] = np.where(both['concat_key_generate_secondary'].isin(list(df_mysql['concat_key_generate_secondary'])) , 1, 0)
        except:
            return {'update_mysql':[],'insert_mysql':glob_comparate['not_exist_mysql'],'delete_mysql':old['only_old']}
        ...
        if not_exist_mysql_s.empty:
            ...
            print(not_exist_mysql_s[not_exist_mysql_s['platform_deviceID']=='ORBITHB600301'],'not_exist_mysql_s_comnot_exist_mysql_s_comnot_exist_mysql_s_comnot_exist_mysql_s_com')
            data_mysql_not_exist_s = df_mysql[df_mysql['concat_key_generate'].isin(list(not_exist_mysql_s_com['concat_key_generate']))]
    ...
    def comparate_secondary_mongo_equals(df_mongo,comparate,old):
        ...
        if not_exist_mongo_s.empty:
            ...
            print(df_mongo[df_mongo['mongo_deviceID']=='ORBITHB600263']['concat_key_generate'],'mongo_deviceIDmongo_deviceIDmongo_deviceID')
