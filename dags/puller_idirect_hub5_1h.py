...
def puller_idirect_hub5_1h():
    ...
    def extract_mysql(engine, config, valid_puller_runing):
        if valid_puller_runing is None:
            return []
        query = (
            "SELECT  "+table_mysql_puller+".id,CAST(latitud AS CHAR(100)) as 'latitud',CAST(longitud AS CHAR(100)) as 'longitud' ,siteId,esn,statusTerminal,did,CAST(id_nms AS INT) as 'id_nms' ,modeltype,inroutegroupId,networkId,ms.name as 'crmId'  FROM "
